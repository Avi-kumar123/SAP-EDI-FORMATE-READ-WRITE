...
public class Message012 extends MessageCommon implements MessageSpecific {
	...
	private String processBody_2B(ResultSet rsCF, String msgType) {
		...
		setQuery("EXEC [dbo].[SP_EMIZA_GET_ARTSANA_MESSAGE_STOCK]");
			...
			while (rsBody.next()) {
				...
				while (rsCF.next()) {
					returnValue += buildColumnValue(rsCF.getString(7), rsCF.getString(5), rsCF.getInt(6),
							rsCF.getString(8),0);
					System.out.println(rsCF.getString(7)+":"+rsCF.getString(5)+":"+rsCF.getInt(6)+":"+rsCF.getString(8));
				}
				recordCount++;
